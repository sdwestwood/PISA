...
In some cases, you would be able to collect new data to perform cross-validation, i.e., see how well the model fitted to your original data fits to newly collected data. In others, it may be unfeasible to collect more data, but you are still interested in what would happen if you had more data from the same data collection process, i.e., if your sample size was larger. 

Essentially, sometimes you are interested in how well your model would perform on unseen data, without the ability or motivation to collect that data. If you are in this situation, there are methods that can be used to cross-validate your model.

## Holdout Cross-Validation
Cross-validation methods that do not require new data collection are based on the idea of a *holdout* sample. You split your data into two parts: a training set and a testing (or *holdout*) set. A model is fitted on the training set and 'tested' on the holdout set. The idea of 'testing' here refers to checking how well the model fits to the holdout sample. If the model fits well on both samples, this offers evidence for the generalisability of your model to a hypothetical larger sample of data (REF). It will tend to fit well on the training sample, as that is the sample from which the model parameters were derived.

Typically, a training set is over half of the original sample (say, 80%). So, only a small proportion of the full dataset is used to test the model fitted to the training set. *DISCUSSION ABOUT PROPORTIONS*

<div id = "holdoutDiv" style = "position: relative;"></div>

## Repeated Cross-Validation
The simple holdout algorithm can be expanded to include iterations, i.e., you can separate your dataset into testing and training data more than once, in different ways. Metrics of interest can be averaged across iterations, the result of which can be used to draw conclusions about the generalisability of the model (REF). By iterating over cross-validation algorithms, the bias of model-fit indicators (e.g., RMSEA, BIC) is reduced compared to simple, singular holdout cross-validation (REF).
...
A popular method of repeated cross-validation is the **$k$-folds** algorithm by which data-points are randomly partitioned into $k$ sub-samples. One after another, each sub-sample is used as the testing set for a model fit to the data in all other sub-samples. See below for a demonstration of the k-folds algorithm. *DISCUSS VALUE OF K*
...
## Applying Cross-Validation in R
...
Using the model fitted to the training data, we need to calculate predictions for the values of $y$ in the testing data based on the observed values of $x$ in the testing data and the parameters $\beta_0$ and $\beta_1$ from the model fitted above. 
...
However, in this case we are not interesting in overfitting. We want to know the results of our repeated cross-validation. To do that, we can average the $R^2$ value (or any other fit index) over iterations. 
