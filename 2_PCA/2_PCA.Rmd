...
```{r pca functions, echo=FALSE}
# Create Biplot Function 
ggbiplot <- function(pcobj, choices = 1:2, scale = 1, pc.biplot = TRUE, 
                      obs.scale = 1 - scale, var.scale = scale, 
                      groups = NULL, ellipse = FALSE, ellipse.prob = 0.68, 
                      labels = NULL, labels.size = 3, alpha = 1, 
                      var.axes = TRUE, line_color = "dodgerblue", lab_color = "black",
                      lab_alpha = 1,
                      ...
                      varname.abbrev = FALSE, ...)
{
 ...
 if(inherits(pcobj, 'prcomp')){
  ...
  if(!is.null(df.u$labels)) {
    if(!is.null(df.u$Country)) {
      g <- g + geom_text(aes(label = labels, color = Country), 
                         size = labels.size)
    } else {
    ...
    if(!is.null(df.u$Country)) {
      g <- g + geom_point(aes(color = Country), alpha = alpha)
    } else {
  ...
  if(var.axes) {
    ...
    geom_text(data = df.v, 
              ...
              color = lab_color, size = varname.size, alpha = lab_alpha)
  }

  return(g + theme_minimal())
}

#Create screeplot function
...
To illustrate the concept, we will choose a couple of plausibly related variables from the PISA dataset: `Bullying` and `Discipline` scores. 

```{r PCA_data, echo=TRUE, message=FALSE, eval=FALSE}
#load in data
