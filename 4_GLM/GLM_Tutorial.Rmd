...
None of us can know the future, but statisticians have the magic power to predict it!
Their super power is called "regression analysis". Basically, what regression analysis does is to fit a model to data and use it to predict values of the dependent variable (DV) from one or more independent variables (IVs). 
...
Now that we have joined the two tables, we can move on to our analysis. The main point that should be gained from this section is that it is important to standardize the data before performing a complex analysis. 
...
ggplot(read_data, aes(Score)) + 
  geom_histogram(binwidth = 50, color = 'white', fill = 'dodgerblue') +
  theme_minimal()
```
...
And now let's start with the analysis!
...
Firstly, we will create a model with a single predictor. 
For example, if we want to know how the Spending variable predicts the Score, we have to build our analisys in this way: 

```{r linear model, with Spending as predictor}
# Use the lm function to create a linear model with a Spending as predictor
LM_mod1 <- lm(Score ~ Spending, data = read_data)
summary(LM_mod1)
```

Interpretation: 
Let’s look first at the R2 statistics. This value describes the overall model (so it tells us whether the model is successful in predicting “Score”). The value of R2 is a measure of how much of the variability in the outcome is accounted for by the predictors. For the first model its value is .105 which means that “Spending” accounts for 10.5% of the variation in “Scores”. That means that 89.5%  of the variation in “Score” can not be explained by advertising alone. Therfore, there must be other variables that have an influence also. The result of ANOVA for these data is significant, so the model results in a significantly good degree of prediction of the outcome variable. However, the ANOVA doesn’t tell us about the individual contribution of variables in the model (in this simple case, only one variable). 

Coefficients contains the model parameters (the beta values) and the significance of these values.  In regression equation, b0  was the Y intercept and this values is the value in the Estimate  colum for the (intercept). It is in brackets because it’s not a real variable. We can say that b0  is 485.44, and this can be interpreted as meaning that wen no “Spending” is done (when X= 0), the model predicts that 485.44 Score would be achieve. We can also read off the value of b1 from the row labelled Spending and this value represent the gradient of the regression line. It is 37.64. Although this value is the slope of the regression line, it is more useful to think of this value as representing the change in the outcome associated with a unit change in the predictor. Therefore, if our predictor variable (“Spending”) is increased by one unit, our model predicts that “Score” will increase 37.63. We also see that the t-test tells us whether the b-value is different from 0. For this variable, we can conclude that the “Spending” makes a significant contribution (p<.001) to predicting “Score”.

We can see in “Spending” raw.

The adjusted R2 gives us some idea of how well our model generalizes, and ideally we would like its value to be the same, or very close to, the value of R2. In this example the difference for the model is small (0,0003)

<p>&nbsp;</p>
<p>&nbsp;</p>

```{r linear model, with ATTLNACT as predictor}
# Use the lm function to create a linear model with a ATTLNACT as predictor
LM_mod2 <- lm(Score ~ ATTLNACT, data = read_data)
summary(LM_mod2)
```


What is have explained for the former linear regression could be applied to that.Do this model explain more or less of the variability in the outcome “Score”?

<p>&nbsp;</p>
<p>&nbsp;</p>

Well, you know, reality is a much more complex model. So we need to build models with more than a single predictor. It is certainly possible that taken together Spending and ATTLNACT explain more about Score than either predictor alone. 

```{r linear model with 2 variable}
# Use the lm function to create a linear model with 2 variables as predictors
LM_mod3 <- lm(Score ~ Spending + ATTLNACT, data = read_data)
summary(LM_mod3)
```


When the two predictors are included, the value of R2 (how much of the variability in the outcome is accounted for by the predictors) increase from 0.105 to 0.108. The b-values tell us that there is a significant positive relationship between both predictors and the outcome (both values are positive). 
Moreover, the b-values tell us to what degree each predictor affects the outcome if the effects of all other predictors are held constant: 1) “Score” would increases by 485.05 when “Spending” increases by one unit; 2) “Score” would increases by 5.548 when “ATTLNACT” increases by one unit. 
Both b-values are making a significant contribution to the model since corresponding t-test are significant. The larger value of t, the greater the contribution of that prediction. In this case, we can see that the “Spending” has greater impact than “ATTLNACT” in “Score”. 

In the next step we will calculate interaction of both variables to know the contribution of it. 

<p>&nbsp;</p>

```{r linear model interaction}
# Use the lm function to create a linear model interaction
LM_mod4 <- lm(Score ~ Spending * ATTLNACT, data = read_data)
summary(LM_mod4)
```

Including the interaction, the value of R2 has increased from 0.108 to 0.11. The b-values tell us that there is a significant positive relationship between both predictors and their interaction with the outcome. If we see the value of t-test, the “Spending” is the factor with the higher contribution to the model. 

<p>&nbsp;</p>
<p>&nbsp;</p>

Let's try to include all the collected variables.

```{r linear model}
# Use the lm function to create a linear model interaction
LM_mod5 <- lm(Score ~ Spending * Learning * ESCS * ATTLNACT + Item,
               data = read_data)
summary(LM_mod5)
```
...
# Specify the random effetcs and thier interactions
                     ...
                     (1 | Item),
                   data = read_data,
# specify the family you want to use and the link function
                   family = Gamma(identity),
# Change the optimizer used in the example
                   control = glmerControl(optimizer = c("bobyqa")))
